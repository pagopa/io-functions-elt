...
const producerMock = {
  ...
  send: vi.fn(async (pr: ProducerRecord) =>
    pipe(
      pr.messages,
      RA.map(
        __ =>
...
const dummyThirdPartyDataWithCategoryFetcher: F.ThirdPartyDataWithCategoryFetcher = vi
  .fn()
  .mockImplementation(serviceId =>
    serviceId == aServiceId ? TagEnumPN.PN : TagEnumBase.GENERIC
  );

// const buildAvroMessagesObject = vi.spyOn(F, "buildAvroMessagesObject");
